...
return {
	...
	keys = {
		...
		{ "ghx",             function() Snacks.gitbrowse() end,                                      mode = { "n", "t" } },
	},
	---@type snacks.Config
	opts = {
	...
	init = function()
		...
		vim.api.nvim_create_autocmd("LspProgress", {
			...
			callback = function(ev)
				...
				local idx = vim.iter(p):enumerate():find(function(_, item)
					return item.token == ev.data.params.token
				end) or (#p + 1)
				...
				local msg = vim.iter(p):map(function(item) return item.msg end):totable()
				progress[client.id] = vim.iter(p):filter(function(item) return not item.done end):totable()
