...
(defconst my-state-directory
  ...
  "Directory for state generated by this configuration.")
...
(defconst my-eglot-server-executables
  '((clojure-ts-mode "clojure-lsp")
    ...
    (python-ts-mode "rass" "pylsp" "pyls"
                    "basedpyright-langserver" "pyright-langserver"
                    "pyrefly" "ty" "jedi-language-server" "ruff"
                    "ruff-lsp")
    (zig-mode "zls"))
  "Executables that can provide an Eglot server for each major mode.")
...
(defun my-eglot-ensure ()
  ...
  (when-let* ((executables
               (alist-get major-mode my-eglot-server-executables)))
    (when (seq-some #'executable-find executables)
      (eglot-ensure))))
